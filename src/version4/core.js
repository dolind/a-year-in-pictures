...
export function diither(src, W, H, dst, doEq) {
    ...
    normalizeGrays(buffers.tmp1, W, H, 64, doEq, buffers.mask);
    ...
    for (let row = 0; row < H; row++) {
        for (let col = 0; col < W; col++) {
            ...
            let px = buffers.tmp1[idx];       // 0..255
            let q = Math.floor((px / 255) * 63);
            let baseLevel = Math.floor(q / 8);
            let remainder = q % 8;

            if (remainder > threshold && baseLevel < 7) baseLevel++;
            dst[idx] = baseLevel;
        }
    }
...
export function diidify(mask, pix, W, H) {
    for (let col = 0; col < W; col++) {
        for (let row = 0; row < H; row++) {
            const idx = col * H + row;          // column-major index into mask
            const rgbaIndex = (row * W + col) * 4; // ImageData is row-major

            // mask[idx] should be 0–7 now
            const level = mask[idx];

            // Scale to 0–255 gray
            const gray = Math.round((level / 7) * 255);

            pix[rgbaIndex] = gray; // R
            pix[rgbaIndex + 1] = gray; // G
            pix[rgbaIndex + 2] = gray; // B
            pix[rgbaIndex + 3] = 255;  // A
        }
    }
}
...
export function ditherIt(img, {exp, color, gray}) {
    ...
    const exposures = [
        0, 0.1, -0.1, -0.05, 0.05, 0.15, -0.15, 0.23, -0.23,
    ].slice(0, (exp - 1) * 2 + 1);
